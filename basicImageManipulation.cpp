...
Image scaleNN(const Image &im, float factor) {
  ...
  int newWidth = floor(im.width() * factor); 
  int newHeight = floor(im.height() * factor); 
  Image output(newWidth, newHeight, im.channels()); 

  for(int x = 0; x < newWidth; x++){
    for(int y = 0; y < newHeight; y++){
      for(int z = 0; z < im.channels(); z++){
        output(x, y, z) = im(x/factor, y/factor, z); 
      }
    }
  }
  return output;
...
float interpolateLin(const Image &im, float x, float y, int z, bool clamp) {
  ...
  // x coordinates 
  int lowerX= floor(x);
  float upper_dx = abs(x - lowerX);
  int upperX = ceil(x);
  float lower_dx = 1 - upper_dx;

  // y coordinates
  int lowerY = floor(y);
  float upper_dy = abs(y - lowerY);
  int upperY = ceil(y); 
  float lower_dy = 1 - upper_dy;

  float E = im.smartAccessor(lowerX, upperY,z, clamp)*lower_dx + im.smartAccessor(upperX, upperY,z, clamp)*upper_dx;
    
  float F = im.smartAccessor(lowerX, lowerY,z,clamp)*lower_dx + im.smartAccessor(upperX, lowerY,z, clamp)*upper_dx;

  float interpolated_y_value = E*upper_dy + F*lower_dy;
...
Image scaleLin(const Image &im, float factor) {
  ...
  for(int x = 0; x < newWidth; x++){
    for(int y = 0; y < newHeight; y++){
      for(int z = 0; z < im.channels(); z++){
        output(x, y, z) = interpolateLin(im, float(x)/factor, float(y)/factor, z, true);
      }
    }
  }
  return output;
...
Image rotate(const Image &im, float theta) {
  ...
  for(int x = 0; x < im.width(); x++){
    for(int y = 0; y < im.height(); y++){
      float rX = x - centerX; 
      float rY = y - centerY; 
      float hypotnuse = sqrt(pow(rX, 2) + pow(rY, 2)); 
      
      // with respects to center X and Y
      float wrcX = cos(theta) * hypotnuse + centerX; 
      float wrcY = sin(theta) * hypotnuse + centerY;


      for(int z = 0; z <im.channels(); z++){
        output(x,y,z) = interpolateLin(im, wrcX, wrcX, z, true);
      }
    }
  }
